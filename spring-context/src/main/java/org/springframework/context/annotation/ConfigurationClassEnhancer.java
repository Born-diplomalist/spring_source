...
class ConfigurationClassEnhancer {
	...
	private Enhancer newEnhancer(Class<?> configSuperClass, @Nullable ClassLoader classLoader) {
		...
		//设置父类（目标类），cglib作为目标类的子类存在
		enhancer.setSuperclass(configSuperClass);
		//增强接口，EnhancedConfiguration是一个很强大的接口，实现了BeanFactoryAware，可以得到BeanFactory 
		...
		// 设置该变量的值为当前Context中的beanFactory.这样我们这个cglib代理的对象就有了BeanFactory
		//有了factory就能获得对象,不用通过方法获得对象,因为通过方法获得对象不能控制其过程
		//该BeanFactory的作用是this调用时拦截此调用，并直接在BeanFactory上获得目标bean
	...
	private static class BeanMethodInterceptor implements MethodInterceptor, ConditionalCallback {
		...
		public Object intercept(Object enhancedConfigInstance, Method beanMethod, Object[] beanMethodArgs,
			...
			//判断类是不是一个FactoryBean，是则返回该对象（通过前缀判断，前缀值为 &）
			//再判断是不是ScopedProxyFactoryBean类型，如果不是在getObjects方法中再进行一次代理以确保该bean是单例的
			//ScopedProxyFactoryBean类型暂时没有代码实现
			...
			if (factoryContainsBean(beanFactory, BeanFactory.FACTORY_BEAN_PREFIX + beanName) &&
				...
				//如果是Scope代理的FactoryBean
				if (factoryBean instanceof ScopedProxyFactoryBean) {
					// Scoped proxy factory beans are a special case and should not be further proxied
				}
				//如果不是ScopedProxyFactoryBean类型，默认单例，在FactoryBean的getObjects方法中在进行一次代理
				else {
					// It is a candidate FactoryBean - go ahead with enhancement
					return enhanceFactoryBean(factoryBean, beanMethod.getReturnType(), beanFactory, beanName);
				}
			}
			//判断方法是不是第一次调用（是new还是getBean），如果是true，则去调用父类方法以new对象（这个父类其实就是被代理对象），不是true则通过$$beanFactory属性中的BeanFactory去getBean
			//判断执行的方法和调用的方法是不是同一个方法（如方法A中调用方法B，在B执行时，A是调用的方法，B是实际执行的方法）
		...
		private Object resolveBeanReference(Method beanMethod, Object[] beanMethodArgs,
			...
			try {
				//如果已经被创建了，设置一个标识，下次有标识就不再直接new而是通过工厂获取
